...
var shiftChar = function (c, shift) {
    ...
    var crypCharIndex = alphabet.indexOf(c);
    if (crypCharIndex != -1) {
        var uncrypCharIndex = crypCharIndex - shift;
        if (uncrypCharIndex < 0) {
            uncrypCharIndex += 26;
        }
        var result = alphabet.charAt(uncrypCharIndex);
        return result;
    }
    else {
        return c;
...
var shiftString = function (str, shift) {
    ...
    for (var i = 0; i < str.length; i++) {
        nextresult += shiftChar(str[i], shift);
    }
    return nextresult;
};
...
document.getElementById("submit").addEventListener('click', function () {
    ...
    var result = caesarDecipher(text, shift);
