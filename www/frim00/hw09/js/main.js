...
var shiftChar = function (cryptedChar, shift) {
    var cryptedCharIndex = alphabet.indexOf(cryptedChar);
...
var shiftString = function (cryptedMessage, shift) {
    var decryptedMessage = "";
    for (var i = 0; i < cryptedMessage.length; i++){
        decryptedMessage += shiftChar(cryptedMessage[i], shift);
    }
...
var caesarDecipher = function (cipherText, usedKey) {
    if (usedKey < 1 || usedKey > 25){
        return alert("invalid cipher!");
    }
...
decipherBtn.onclick = function() {
    ...
    var cipherInput = document.querySelector("input[type=number]").value;
