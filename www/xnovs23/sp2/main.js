var cardFilesModule = (function () {
    ...
    const initUI = function (displayPatientsUI, firstName, lastName, currentUserId) {
        ...
        if (displayPatientsUI) {
            ...
            document.getElementById("formNewPatient").addEventListener('submit', addPatientsToDatabase);
            ...
            document.getElementById("submitBackToPatientsButton").addEventListener("click", goBackToPatientList);
    ...
    const goBackToPatientList = function (truth) { 
                initUI(true);
            };
    ...
    const loadUsers = function (callbackForReceivingUsers) { 
        let stringyfiedUsers = window.localStorage.getItem("users");
        if (stringyfiedUsers) {
 
            console.log("ide to z local storagu");
            console.log(stringyfiedUsers);
 
            callbackForReceivingUsers(stringyfiedUsers);
            return; 
        }
 
        console.log("nemam v local storage musim na firebase");
    ...
    const addPatientsToDatabase = function (e) { 
        e.preventDefault();
        console.log(123)
        ...
        sendRequestToFirebase("PUT", firebaseUrlUsers + userIdentifier + ".json", function () { 
            ...
            let stringyfiedUsers = window.localStorage.getItem("users"); 
            let parsedUsers = JSON.parse(stringyfiedUsers); 
 
            parsedUsers[userIdentifier] = userFromForm; 
            let stringyfyUsersBack = JSON.stringify(parsedUsers); 
            window.localStorage.setItem("users", stringyfyUsersBack); 
    ...
    const addRowToPatientList = function (firstName, lastName, creationDate, patientId) {
        let existingRow = document.getElementById("row-" + patientId);
        if (existingRow) 
        {
            return;
        }
    ...
    const editPatientInPatientList = function (responseData) {
        ...
        parsedUsers[context.patientIdentifierForEdit] = parsedEditedData; 
        let stringyfyUsersBack = JSON.stringify(parsedUsers); 
        window.localStorage.setItem("users", stringyfyUsersBack); 
    ...
    const addExerciseToExerciseList = function (date, note, exerciseId) {
        let existingRowExercise = document.getElementById("row-" + exerciseId);
        if (existingRowExercise)
        {
            return;
        }
    ...
        const getUserFromForm = function () {
        ...
                if (formPatientFirstName.length === 0) { 
                    alert ("Vyplňte krstné meno pacienta.");
                    return;}
                let formPatientLastName = document.getElementById("patient-last-name").value;
                if (formPatientLastName.length === 0) { 
                    alert ("Vyplňte priezvisko pacienta.");
                    return;}
                let formCreateDate = document.getElementById("create-date").value;
                if (formCreateDate.length === 0) { 
                    alert ("Zadajte dátum založenia pacienta do kartotéky.");
                    return;}
