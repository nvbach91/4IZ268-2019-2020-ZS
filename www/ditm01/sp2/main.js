...
App.startInput = $('#datepicker');
App.endInput = $('#end-picker');
...
App.updateSigninStatus = function (isSignedIn) {
    if (isSignedIn) {
        ...
        var userName = `<p class="user-info"> Welcome, ${profile.getBasicProfile().getName()} </p>`;
        App.userPanel.append(userName);
        ...
        signoutButton.className = 'signed-out';
    };
};
...
var startTimeValue = App.startTime.val();
var endTimeValue = App.endTime.val();

App.initEvent = function () {
    App.eventForm.submit(function (event) {
        event.preventDefault();
        var titleValue = App.titleInput.val();
        var startTime = new Date(startPicker);
        var endTime = new Date(endPicker);
...
App.displayDetails = function (name) {
    ...
    App.sendButton.click(function () {
        ...
        request.execute(function () {
            ...
            App.updageLocalEvents();
            App.message.append('your event has been successfully created');
            setTimeout(function (){
                App.message.empty(); 
            }, 3000)
        });
